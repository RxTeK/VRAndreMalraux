    ...
    const FHitResult& SweepResult)
{
  ...
  if (InputMethod == EIsdkGrabInputMethod::Pinch && !GrabberComponent->IsPinchGrabAllowed())
  {
    return;
  }

  // If grabber doesn't allow palm grabs and this is a palm grab collider, don't hover
  if (InputMethod == EIsdkGrabInputMethod::Palm && !GrabberComponent->IsPalmGrabAllowed())
  {
    return;
  }
